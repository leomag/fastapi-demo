...
def add_income(
    operation: OperationRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
...
def get_operations_list(
    ...
    return operation_service.get_operations_list(db, user, wallet_id, date_from, date_to)
...
async def create_transfer(
    ...
    return await operation_service.transfer_beetween_wallets(db, user.id, payload)
